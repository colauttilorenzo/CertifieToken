...
contract Managed is Ownable {
    ...
    constructor(address provider_) Ownable(msg.sender) {
        _provider = provider_;
    }
     ...
     */
    function provider() public view virtual returns (address) {
        return _provider;
    }

    /**
     * @dev Throws if the sender is not the owner or provider.
     */
    function _checkOwner() internal view virtual override(Ownable) {
        if (owner() != _msgSender() && provider() != _msgSender()) {
            revert OwnableUnauthorizedAccount(_msgSender());
        }
    }
    ...
    function renounceOwnership() public virtual override(Ownable) onlyOwner {
        _transferOwnership(_provider);
    }
}

contract CertifieToken is ERC721, Managed {
    using Counters for Counters.Counter;

    string private _metadata;

    int256 private _expiring_ts;
    uint256 private _created_at;
    ...
    constructor(
        ...
        int256 expiring_ts_
    ) ERC721(name_, symbol_) Managed(provider_) {
        _metadata = metadata_;
        _expiring_ts = expiring_ts_;
    ...
    function safeMint(
        ...
        _safeMint(to, tokenId);
        _setTokenInfo(tokenId, hash_, tokenUri);
    }
    ...
    function isExpired() public view returns (bool) {
        bool test = false;
        if (_expiring_ts > -1) {
            test = block.timestamp >= (_created_at + uint256(_expiring_ts));
        }
